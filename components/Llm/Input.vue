<script setup lang="ts">
import { useTextareaAutosize } from "@vueuse/core";

const emit = defineEmits<{
  input: [input: string];
}>();

// Callback to trigger `sent` event, so the parent can react accordingly.
const onInput = () => {
  if (input.value === "") return;
  emit("input", input.value);
  input.value = "";
};

// Send input on "Enter", but not "Shift+Enter".
const onKeyDown = (event: KeyboardEvent) => {
  if (event.key === "Enter" && !event.shiftKey) {
    event.preventDefault();
    onInput();
  }
};

// Have the textarea autosize.
const { textarea, input } = useTextareaAutosize();
// `input.value`'s initial value is `undefined`. Set it to `''` so that the button recognizes it as empty input.
input.value = "";
</script>

<template>
  <div
    class="flex min-h-[3.5rem] min-w-[30rem] items-center rounded-[1.75rem] bg-neutral-800"
  >
    <div class="flex h-full shrink-0 grow basis-[20rem] items-center px-4 py-2">
      <textarea
        ref="textarea"
        v-model="input"
        class="max-h-full h-[1.5rem] grow basis-0 resize-none bg-neutral-800 text-base text-white placeholder:text-base focus-visible:outline-none"
        placeholder="Chat with LLM"
        @keydown="onKeyDown"
      ></textarea>
    </div>
    <button
      class="mr-2 size-[2.5rem] flex-none rounded-full p-2"
      :class="
        input === '' ? ['bg-neutral-500'] : ['bg-white', 'hover:bg-neutral-300']
      "
      :disabled="input === ''"
      @click="onInput"
    >
      <svg
        class="fill-neutral-700"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
      >
        <g id="_01_align_center" data-name="01 align center">
          <path
            d="M1.444,6.669a2,2,0,0,0-.865,3.337l3.412,3.408V20h6.593l3.435,3.43a1.987,1.987,0,0,0,1.408.588,2.034,2.034,0,0,0,.51-.066,1.978,1.978,0,0,0,1.42-1.379L23.991.021ZM2,8.592l17.028-5.02L5.993,16.586v-4Zm13.44,13.424L11.413,18h-4L20.446,4.978Z"
          />
        </g>
      </svg>
    </button>
  </div>
</template>
